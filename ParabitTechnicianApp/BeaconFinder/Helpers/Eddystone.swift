...
class BeaconInfo : NSObject {
  ...
  class func frameTypeForFrame(advertisementData: [NSObject : AnyObject])
    -> EddystoneFrameType {
      if let advertisementFrameList = advertisementData[CBAdvertisementDataServiceDataKey as NSObject]
        ...
        if let frameData = advertisementFrameList[uuid] as? NSData {
          if frameData.length > 1 {
            ...
            if let serviceUUIDs = advertisementData[CBAdvertisementDataServiceUUIDsKey as NSObject] {
              for service in serviceUUIDs as! NSArray {
                let stringServiceUUID = String(describing: service)
                if GATTServiceID.compare(stringServiceUUID, options: .caseInsensitive) ==
