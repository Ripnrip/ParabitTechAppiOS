...
class BeaconItem {
  ...
  init(peripheral: CBPeripheral, frames: BeaconFramesSupported, operations: GATTOperations) {
    ...
    if let UID = frames.UIDData {
        topView = CustomViews.createFrameDisplayView(type: .UIDFrameType,
                                                   data:UID,
                                                   holder: self.framesView,
                                                   topView: topView,
                                                   pinTop: pinTop,
                                                   setBottomConstraints:
            UIDNeedBottomConstraints(frames: frames))
    }
    ...
    if let EID = frames.EIDData {
      if topView != self.framesView {
        ...
        topView = CustomViews.createFrameDisplayView(type: .EIDFrameType,
                                                   data: EID,
                                                   holder: self.framesView,
                                                   topView: topView,
                                                   pinTop: pinTop,
                                                   setBottomConstraints:
            EIDNeedBottomConstraints(frames: frames))
    }
    ...
    if let URL = frames.URLData {
      if topView != self.framesView {
        ...
        topView = CustomViews.createFrameDisplayView(type: .URLFrameType,
                                                   data:URL,
                                                   holder: self.framesView,
                                                   topView: topView,
                                                   pinTop: pinTop,
                                                   setBottomConstraints:
            URLNeedBottomConstraints(frames: frames))
    }
    ...
    if let TLM = frames.TLMData {
      if topView != self.framesView {
        ...
        topView = CustomViews.createFrameDisplayView(type: .TelemetryFrameType,
                                                   data: TLM,
                                                   holder: self.framesView,
                                                   topView: topView,
                                                   pinTop: pinTop,
                                                   setBottomConstraints:
            TLMNeedBottomConstraints(frames: frames))
    }
    ...
    if frames.EddystoneGATTServiceFrameSupported {
      if topView != self.framesView {
        ...
        CustomViews.displayConfigurableBeacon(holder: self.framesView, topView: topView, pinTop: pinTop)
    }
  }
